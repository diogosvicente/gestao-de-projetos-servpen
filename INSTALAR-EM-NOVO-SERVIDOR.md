...
## 0.3. Redirect 228.20 → 238.40 (kill switch via `.env`)

Durante a janela de transição, você pode querer que **TODO acesso ao
228.20 seja redirecionado pro 238.40** sem precisar mexer em DNS, sem
desligar o app antigo, e podendo voltar atrás em segundos se algo
quebrar no servidor novo.

A regra já está embutida no vhost Apache (`gestao-de-projetos.conf`).
A ativação/desativação é controlada pela existência de um arquivo:

```
/etc/gestao-de-projetos/redirect-to-238.env
```

Existe → redireciona. Não existe → app antigo serve normalmente.

### Ativar o redirect (no 228.20)

```bash
sudo cp /var/www/setup-novo-servidor/redirect-to-238.env.example \
        /etc/gestao-de-projetos/redirect-to-238.env

# Edite as notas (opcional — o Apache ignora o conteúdo, é só pra documentação):
sudo nano /etc/gestao-de-projetos/redirect-to-238.env
```

A partir desse momento, qualquer request pra
`http://152.92.228.20/gestao-de-projetos/...` retorna HTTP 302 e o
browser segue pra `http://152.92.238.40/gestao-de-projetos/...` com
o **path preservado** (a querystring também — `[QSA]` no
`RewriteRule`).

**Sem reload do Apache.** O vhost usa `RewriteCond -f` que testa a
existência do arquivo a cada request — custo desprezível (stat() é
cacheado pelo kernel).

### Desativar o redirect (no 228.20)

```bash
sudo rm /etc/gestao-de-projetos/redirect-to-238.env
```

A partir desse momento, requests caem de volta no app antigo. **Sem
reload do Apache.**

### Por que 302 e não 301?

301 (Moved Permanently) é cacheado AGRESSIVAMENTE pelo browser e
proxies intermediários. Mesmo após você fazer `rm` do arquivo, browsers
de quem visitou durante a janela ativa continuariam sendo redirecionados
por dias/semanas. **302** (Found, temporário) não é cacheado por
padrão — liga/desliga limpo.

### Atenção: jamais criar este arquivo no 238.40

O redirect aponta pro 238.40. Se por engano alguém criar
`/etc/gestao-de-projetos/redirect-to-238.env` no servidor 238.40,
qualquer request pra ele vai redirecionar pra ele mesmo → **loop
infinito**. O navegador trava com "ERR_TOO_MANY_REDIRECTS".

O nome do arquivo é deliberadamente explícito (`redirect-to-238.env`)
pra deixar isso óbvio: ninguém no servidor 238.40 cria um arquivo
chamado "redirect to 238". Mas vale a vigilância.

### Validação

Com o redirect ativo, do seu terminal:

```bash
# Esperado: HTTP/1.1 302 Found + Location: http://152.92.238.40/gestao-de-projetos/
curl -sI http://152.92.228.20/gestao-de-projetos/

# Esperado: vai seguir pro 238.40 e mostrar a página de login
curl -sIL http://152.92.228.20/gestao-de-projetos/
```

Com o redirect desativado:

```bash
# Esperado: HTTP/1.1 200 OK (resposta direta do Streamlit via proxy)
